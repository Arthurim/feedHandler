...
from persistence import get_args_for_subscription, persist_subscription_result_to_kdb
...
def create_wss_connection_url_for_market(subscription_type,market,sym):
    if market=="KRAKEN":
        ...
        if subscription_type=="orderbooks":
            ...
            raise ValueError("This subscription_type is not yet supported: "+str(subscription_type) +" for market: "+ market)
    elif market=="BINANCE":
        if subscription_type=="orderbooks":
...
def create_ws_subscription_logger(subscription_type,sym,market):
    ...
    logFile = 'C:/dev/log/marketdata/log_'+subscription_type+'_'+market+"_"+datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S-%f")
...
def create_ws_subscription_kdb_persister(subscription_type, sym, market, depth=10):
    ...
    i = 0
    # while datetime.datetime.now().day<21:
    while i < 10:
        i = i + 1
        try:
            result = ws.recv()
