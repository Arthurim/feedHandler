...
from market_data.src.utils.python_to_kdb_conversion import convert_ohlc_series_to_kdb_row
...
def persist_ohlc_to_kdb(result):
    ...
    new_row = pd.Series({"time": datetime.datetime.now().strftime("%H:%M:%S.%f"),
                         ...
                         "marketTimestamp": datetime.datetime.fromtimestamp(float(ohlc[0])).strftime(
                             "%Y.%m.%dD%H:%M:%S.%f"),
                         "market": "KRAKEN",
